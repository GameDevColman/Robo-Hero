...
public class ArmamentItem : Item
{
    ...
    {
        base.Use();
        Collider[] hits = Physics.OverlapSphere(SceneManagerScript.Instance.thirdPersonController.transform.position, 2f);
        if (hits.Length > 0)
        {
            foreach (Collider hit in hits)
            {
                ...
                {
                    Transform playerTransform = SceneManagerScript.Instance.thirdPersonController.transform;
